...
class Concept(Base, CreatedAtMixin):
    ...
    __table_args__ = (
        ...
        # OpenAlex concept id (e.g. "C277635036") — set by the bulk taxonomy
        # backfill; lets the parent hierarchy be linked by id, not by name.
        # Non-partial so ON CONFLICT (openalex_id) can use it as an arbiter.
        Index("uq_concepts_openalex_id", "openalex_id", unique=True),
    ...
    # OpenAlex parent id, kept so the hierarchy can be re-linked set-based
    # (single UPDATE) instead of one lookup pair per concept.
    parent_openalex_id: Mapped[str | None] = mapped_column(Text)
