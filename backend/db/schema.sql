...
-- Non-partial so ON CONFLICT (openalex_id) can infer it; NULLs stay distinct.
...
-- OpenAlex topics
--
-- OpenAlex's three-level topic taxonomy is a closed, small vocabulary
-- (~4.5k rows) whose hierarchy (domain -> field -> subfield -> topic) is
-- denormalized onto each topic row.  publication_topics and person_topics
-- link works and people to that vocabulary.
-- ---------------------------------------------------------------------------
...
CREATE TABLE organizations (
  ...
  CONSTRAINT organizations_homepage_url_scheme
    CHECK (homepage_url IS NULL OR homepage_url ~* '^https?://')
);
...
CREATE INDEX idx_organizations_short_name_trgm ON organizations USING gin (short_name gin_trgm_ops);
...
CREATE TABLE source_snapshots (
  ...
  CONSTRAINT source_snapshots_source_url_scheme
    CHECK (source_url ~* '^https?://'),
  CONSTRAINT source_snapshots_http_status_range
...
CREATE TABLE external_identifiers (
  ...
  CONSTRAINT external_identifiers_external_id_not_blank CHECK (btrim(external_id) <> '')
);
...
CREATE TABLE embedding_runs (
  ...
  CONSTRAINT embedding_runs_point_count_non_negative CHECK (point_count >= 0)
);
...
CREATE TABLE person_projections_2d (
  ...
  person_id     BIGINT NOT NULL,
  view          TEXT NOT NULL DEFAULT 'topic',
  x             DOUBLE PRECISION NOT NULL,
  y             DOUBLE PRECISION NOT NULL,
  similarity_group  SMALLINT,
  cluster_id    SMALLINT,
  PRIMARY KEY (run_id, person_id, view),
  CONSTRAINT person_projections_2d_view_allowed
    CHECK (view IN ('topic', 'network')),
  CONSTRAINT person_projections_2d_xy_bounded
    CHECK (x BETWEEN -1000000 AND 1000000 AND y BETWEEN -1000000 AND 1000000),
  CONSTRAINT person_projections_2d_cluster_id_non_negative
    CHECK (cluster_id IS NULL OR cluster_id >= 0)
);

CREATE INDEX idx_person_projections_2d_person
  ON person_projections_2d (person_id);

-- Cluster summary + inter-cluster edges for both atlas views.  Positions are
-- fully precomputed offline; the API only reads these rows.
CREATE TABLE projection_clusters (
  ...
  CONSTRAINT projection_clusters_view_allowed CHECK (view IN ('topic', 'network')),
  CONSTRAINT projection_clusters_member_count_positive CHECK (member_count > 0),
  CONSTRAINT projection_clusters_xy_bounded
    CHECK (cx BETWEEN -1000000 AND 1000000 AND cy BETWEEN -1000000 AND 1000000),
  CONSTRAINT projection_clusters_color_slot_non_negative CHECK (color_slot >= 0)
);
...
CREATE TABLE projection_cluster_edges (
  ...
  CONSTRAINT projection_cluster_edges_view_allowed CHECK (view IN ('topic', 'network')),
  CONSTRAINT projection_cluster_edges_no_self CHECK (source_cluster <> target_cluster),
  CONSTRAINT projection_cluster_edges_weights_non_negative CHECK (
