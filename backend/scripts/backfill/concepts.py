...
def link_publication_concepts_batch(
    session: Session,
    publication_id: int,
    concepts: list[tuple[str, int, float]],
) -> int:
    """Link a work's concepts in one round trip (multi-VALUES CTE).

    Creates any missing concept rows inline and joins back for their ids, so
    this replaces the old per-concept upsert+link pair — important on a
    remote Postgres (Neon) where each statement is a separate network hop.
    """
    if not concepts:
        return 0
    values = ", ".join(f"(:n{i}, :l{i}, :s{i})" for i in range(len(concepts)))
    params: dict[str, Any] = {"pid": publication_id}
    for i, (name, level, score) in enumerate(concepts):
        params[f"n{i}"] = name.strip()
        params[f"l{i}"] = int(level)
        params[f"s{i}"] = float(score)
    session.execute(
        text(
            f"""
            WITH ins AS (
                ...
                FROM (VALUES {values}) AS x(name, level)
                ON CONFLICT (display_name) DO UPDATE
                  SET level = LEAST(concepts.level, EXCLUDED.level)
                RETURNING id, display_name
            )
            INSERT INTO publication_concepts (publication_id, concept_id, score)
            SELECT :pid, c.id, x.score
            FROM (VALUES {values}) AS x(name, level, score)
            JOIN concepts c ON c.display_name = x.name
...
def backfill_publication_concepts(
    ...
    *,
    ...
    for row in targets:
        ...
        linked = link_publication_concepts_batch(session, pub_id, concepts)
        if linked:
