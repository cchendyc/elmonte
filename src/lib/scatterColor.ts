...
export type ScatterColorMode = "similarity" | "institution" | "focus";
...
export interface CategoryColorMaps {
  ...
  similarityByGroup: Map<number, string>;
}
...
export function buildCategoryColorMaps(
  ...
  const similarityByGroup = new Map<number, string>();
  const groupSorted = [
    ...new Set(
      points
  ...
  groupSorted.forEach((id, index) => {
    similarityByGroup.set(id, paletteColor(index));
  });

  return { institutionById, similarityByGroup };
}
...
export function colorForSimilarityGroup(
  similarityGroup: number,
  colors: CategoryColorMaps,
): string {
  return colors.similarityByGroup.get(similarityGroup) ?? paletteColor(similarityGroup);
}
...
export function pointFill({
  ...
  if (mode === "focus") {
    ...
    if (point.similarityGroup != null) {
      return colorForSimilarityGroup(point.similarityGroup, colors);
    }
...
export function modeSubtitle(
  ...
  switch (mode) {
    case "similarity":
      return groupCount > 0
        ? `${groupCount} similarity groups · size = publications & citations`
        : "Color = structural similarity · size = publications & citations";
    case "institution":
