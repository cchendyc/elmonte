  ...
  const expand = useCallback(
    async (id: string, advanceTrail = true) => {
      ...
      try {
        ...
        errorRef.current = describeError(err);
        console.error("[graphSession] expand failed", err);
      } finally {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        console.error("[graphSession] roster prefetch failed", err);
      } finally {
  ...
  const onToggleGroup = useCallback(
    async (ownerId: string, key: string) => {
      ...
      try {
        ...
        console.error("[graphSession] fetchPage failed", err);
      } finally {
        dispatch({ type: "loading", id: gid, value: false });
      }
    },
